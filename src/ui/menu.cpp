...
MenuActions MainMenu::drawAndProcess(int screenW, int screenH) {
    ...
    if (currentScreen == MenuScreen::Main) {
        ...
        Widgets::textInput({ centerX - 100, centerY - 20, 200, 44 }, hostPortBuf, sizeof(hostPortBuf), hostPortActive, "7777");
        ...
        Widgets::textInput({ centerX - 140, centerY - 20, 280, 44 }, joinIpBuf, sizeof(joinIpBuf), joinInputActive, "127.0.0.1:7777");

        if (Widgets::button("CONNECT", { btnX, centerY + 45, btnW, btnH }, Colors::Zinc800, Colors::Green500, false, 20)) {
        ...
        // CATEGORY TABS (CURSORS, FLAGS, SCALING & LAYOUT)
        float tabW = 150.0f;
        float tabH = 34.0f;
        float tabGap = 12.0f;
        ...
        TabDef tabs[3] = {
            ...
            { CustomizeTab::Layout,  "SCALING & LAYOUT", -1 }
        };
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            int tw = MeasureText(tTitle.c_str(), 13);
            DrawText(tTitle.c_str(), static_cast<int>(tX + (tabW - tw) * 0.5f), static_cast<int>(tabY + (tabH - 13) * 0.5f), 13, isActive ? Colors::Green400 : (isHover ? WHITE : Colors::Zinc400));
        }

        // TAB SHORTCUT (Tab key cycles through Cursors, Flags, Layout)
        if (IsKeyPressed(KEY_TAB)) {
            activeTab = (activeTab == CustomizeTab::Cursors) ? CustomizeTab::Flags : ((activeTab == CustomizeTab::Flags) ? CustomizeTab::Layout : CustomizeTab::Cursors);
            reelScrollX = 0.0f;
            reelTargetScrollX = 0.0f;
        }

        if (activeTab == CustomizeTab::Layout) {
            // SCALING & LAYOUT PANEL
            float panelW = 580.0f;
            float panelH = 264.0f;
            float panelX = centerX - panelW * 0.5f;
            float panelY = tabY + 44.0f;

            DrawRectangleRec({ panelX, panelY, panelW, panelH }, Colors::Zinc950);
            DrawRectangleLinesEx({ panelX, panelY, panelW, panelH }, 1.5f, Colors::Zinc800);

            float rowX = panelX + 24.0f;
            float rowW = panelW - 48.0f;
            float curY = panelY + 14.0f;
            float rowH = 26.0f;
            float rowGap = 10.0f;
            ...
            curY += rowH + rowGap + 8.0f;

            // 6. Presets and Interactive Rearrange Button
            float btnPresetW = 95.0f;
            float btnPresetH = 30.0f;
            float curBtnX = rowX;
            ...
            curBtnX += btnPresetW + 14.0f;

            float arrangeBtnW = rowW - (curBtnX - rowX);
            if (Widgets::button("DRAG & REARRANGE", { curBtnX, curY, arrangeBtnW, btnPresetH }, Colors::Zinc800, Colors::Green500, false, 13)) {
                isEditingLayoutCanvas = true;
            }

            // CRT Shader Checkbox
            Widgets::checkbox("ENABLE RETRO CRT SHADER", { centerX - 130, panelY + panelH + 16.0f }, crtEnabled, false);

            // DONE BUTTON
            if (Widgets::button("DONE", { btnX, panelY + panelH + 54.0f, btnW, btnH }, Colors::Zinc800, Colors::Green500, false, 20)) {
                currentScreen = MenuScreen::Main;
